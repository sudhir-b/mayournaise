...
Si vous développez une application de production, nous recommandons de mettre à jour la configuration pour activer les règles de lint conscientes du typage :
...
- Vous pouvez également ajouter `...tseslint.configs.stylisticTypeChecked`
...
export default tseslint.config({
  // Définissez la version de react
  settings: { react: { version: '18.3' } },
  plugins: {
    // Ajoutez le plugin react
    react,
  },
  rules: {
    // autres règles...
    // Activez ses règles recommandées
